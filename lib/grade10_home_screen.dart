...
import 'grade10_term1_screen.dart';
import 'grade10_term2_screen.dart';
import 'grade10_term3_screen.dart';
import 'grade10_term4_screen.dart';

class Grade10HomeScreen extends StatefulWidget {
  const Grade10HomeScreen({super.key});

  @override
  State<Grade10HomeScreen> createState() => _Grade10HomeScreenState();
}

class _Grade10HomeScreenState extends State<Grade10HomeScreen> {
  String username = '';
  ...
  void _openTerm(BuildContext context, int term) {
    ...
    switch (term) {
      case 1:
        screen = const Grade10Term1Screen();
        break;
      case 2:
        screen = const Grade10Term2Screen();
        break;
      case 3:
        screen = const Grade10Term3Screen();
        break;
      case 4:
      default:
        screen = const Grade10Term4Screen();
        break;
    }
  ...
  Future<void> _openRevision() async {
    final uri = Uri.parse('https://www.testpapers.co.za/gr10-mathematics');
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not open the revision link')),
      );
    }
  }
          ...
          SafeArea(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  const SizedBox(height: 36), // space under the top-right home button
                  ...
                  Text(
                    "Let’s crush Grade 10 and set you up for a powerful Grade 11.",
                    style: TextStyle(fontSize: 14.5, height: 1.45, color: Colors.black.withOpacity(0.75)),
                  ),
                  if (dreamUniversity.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Text(
                        "Destination: $dreamUniversity 🎯  — every topic you master is a step closer.",
                  ...
                  _TermCard(title: "Term 1", color: Colors.green, onTap: () => _openTerm(context, 1)),
                  const SizedBox(height: 12),
                  _TermCard(title: "Term 2", color: Colors.blueAccent, onTap: () => _openTerm(context, 2)),
                  const SizedBox(height: 12),
                  _TermCard(title: "Term 3", color: Colors.orangeAccent, onTap: () => _openTerm(context, 3)),
                  const SizedBox(height: 12),
                  _TermCard(title: "Term 4", color: Colors.redAccent, onTap: () => _openTerm(context, 4)),
...
class _TermCard extends StatelessWidget {
  ...
  const _TermCard({required this.title, required this.color, required this.onTap});
          ...
          children: [
            ...
            const Expanded(
              child: Text(
                ...
                style: TextStyle(fontSize: 15.5, fontWeight: FontWeight.w700, color: Colors.black87),
              ),
            ),
            Icon(Icons.chevron_right, color: Colors.black.withOpacity(0.6)),
          ],
